/**
 * @pdfsmaller/pdf-decrypt â€” PDF decryption with AES-256 and RC4 support
 ...
 *
 * Implements:
 *   - AES-256 (V=5, R=6) per ISO 32000-2:2020 â€” Algorithms 2.A, 2.B, 11, 12, 13
 *   - RC4 128-bit (V=2, R=3) per ISO 32000-1:2008 â€” Algorithms 2, 4, 5, 7
...
import { md5, RC4, hexToBytes, bytesToHex } from './crypto-rc4.mjs';
import {
  sha256,
...
/**
 * Validate user password â€” Algorithm 4 (Rev 2) / Algorithm 5 (Rev 3)
...
/**
 * Validate owner password â€” Algorithm 7 (Rev 3) / simplified for Rev 2
...
function validateOwnerPasswordRC4(ownerPassword, encryptParams) {
  ...
  if (revision === 2) {
    ...
    // Rev 3: Reverse the 20-iteration RC4 (i = 19 â†’ 0)
 ...
 *
 * Steps:
 * 1. hash2B(password, U[32:40], []) â†’ compare with U[0:32]
 * 2. If match: hash2B(password, U[40:48], []) â†’ decrypt UE â†’ file key
 *
...
async function validateUserPasswordAES256(password, encryptParams) {
  ...
  // Password valid â€” derive file key from UE
  // Key salt = U[40:48]
 ...
 *
 * Steps:
 * 1. hash2B(password, O[32:40], U) â†’ compare with O[0:32]
 * 2. If match: hash2B(password, O[40:48], U) â†’ decrypt OE â†’ file key
 *
 * @returns {Promise<Uint8Array|null>} - 32-byte file key if valid, null if invalid
 */
async function validateOwnerPasswordAES256(password, encryptParams) {
  ...
  // Password valid â€” derive file key from OE
  // Key salt = O[40:48]
...
function collectEncryptedItems(context, encryptRefNum, encryptMetadata) {
  ...
  for (const [ref, obj] of indirectObjects) {
    ...
    // Skip signature dictionaries â€” /Contents and /ByteRange must not be decrypted
...
export async function decryptPDF(pdfBytes, password) {
  try {
    ...
    if (encryptParams.algorithm === 'AES-256') {
      ...
      // Verify Perms (optional but recommended â€” warns but doesn't fail)
      const permsValid = await verifyPerms(fileKey, encryptParams);
      if (!permsValid) {
        // Some PDFs have invalid Perms but are otherwise fine â€” proceed anyway
      }
